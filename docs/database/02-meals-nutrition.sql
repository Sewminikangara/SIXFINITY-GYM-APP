...
CREATE INDEX IF NOT EXISTS idx_meal_plan_items_date ON public.meal_plan_items(specific_date);
...
CREATE INDEX IF NOT EXISTS idx_ai_meal_suggestions_user_id ON public.ai_meal_suggestions(user_id);
CREATE INDEX IF NOT EXISTS idx_ai_meal_suggestions_date ON public.ai_meal_suggestions(user_id, suggested_for_date);
CREATE INDEX IF NOT EXISTS idx_ai_meal_suggestions_active ON public.ai_meal_suggestions(user_id, expires_at) WHERE is_dismissed = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_nutrition_reports_user_id ON public.nutrition_reports(user_id);
CREATE INDEX IF NOT EXISTS idx_nutrition_reports_period ON public.nutrition_reports(user_id, report_year DESC, report_month DESC);
...
CREATE TABLE IF NOT EXISTS public.user_nutrition_badges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    badge_id UUID NOT NULL REFERENCES public.nutrition_badges(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_user_nutrition_badges_user_id ON public.user_nutrition_badges(user_id);
...
CREATE INDEX IF NOT EXISTS idx_barcode_foods_barcode ON public.barcode_foods(barcode);
...
BEGIN
    INSERT INTO public.daily_nutrition_summary (
        user_id, date, total_calories_consumed, total_protein_grams,
        total_carbs_grams, total_fats_grams, total_fiber_grams,
        total_sugar_grams, total_sodium_mg, meals_logged,
        breakfast_logged, lunch_logged, dinner_logged, snacks_count,
        calories_target, protein_target_grams, carbs_target_grams, fats_target_grams
    )
    SELECT 
    ...
    DO UPDATE SET
        ...
        updated_at = NOW(),
        goal_met = (EXCLUDED.total_calories_consumed >= public.daily_nutrition_summary.calories_target * 0.9 
                    AND EXCLUDED.total_calories_consumed <= public.daily_nutrition_summary.calories_target * 1.1),
        protein_goal_met = (EXCLUDED.total_protein_grams >= public.daily_nutrition_summary.protein_target_grams);

    RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trigger_update_daily_nutrition_summary ON public.meals;
...
RETURNS TRIGGER AS $$
BEGIN
    UPDATE public.daily_nutrition_summary
    SET 
        total_water_ml = (
...
DROP TRIGGER IF EXISTS trigger_update_water_summary ON public.water_intake;
...
ALTER TABLE public.nutrition_badges ENABLE ROW LEVEL SECURITY;
...
-- Meal Items Policies
DROP POLICY IF EXISTS "Users can view meal items of their meals" ON public.meal_items;
...
DROP POLICY IF EXISTS "Users can insert meal items to their meals" ON public.meal_items;
...
DROP POLICY IF EXISTS "Users can update meal items of their meals" ON public.meal_items;
CREATE POLICY "Users can update meal items of their meals" ON public.meal_items FOR UPDATE 
    USING (meal_id IN (SELECT id FROM public.meals WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can delete meal items from their meals" ON public.meal_items;
CREATE POLICY "Users can delete meal items from their meals" ON public.meal_items FOR DELETE 
    USING (meal_id IN (SELECT id FROM public.meals WHERE user_id = auth.uid()));
...
DROP POLICY IF EXISTS "Users can manage their own nutrition summary" ON public.daily_nutrition_summary;
...
DROP POLICY IF EXISTS "Users can manage their shopping list items" ON public.shopping_list_items;
...
DROP POLICY IF EXISTS "Users can view their own meal suggestions" ON public.ai_meal_suggestions;
CREATE POLICY "Users can view their own meal suggestions" ON public.ai_meal_suggestions FOR ALL 
    USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Nutrition Reports Policies
DROP POLICY IF EXISTS "Users can view their own nutrition reports" ON public.nutrition_reports;
...
-- Nutrition Badges Policies (Read-only for all authenticated users)
DROP POLICY IF EXISTS "Authenticated users can view all badges" ON public.nutrition_badges;
...
DROP POLICY IF EXISTS "Users can view their own earned badges" ON public.user_nutrition_badges;
CREATE POLICY "Users can view their own earned badges" ON public.user_nutrition_badges FOR ALL 
    USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Barcode Foods Policies (Read-only for all authenticated users)
DROP POLICY IF EXISTS "Authenticated users can view barcode foods" ON public.barcode_foods;
...
DO $$ 
BEGIN
    RAISE NOTICE 'Meals & Nutrition tables created successfully!';
    RAISE NOTICE '   - 14 tables: meals, meal_items, water_intake, daily_nutrition_summary';
    RAISE NOTICE '   - meal_plans, meal_plan_items, shopping_lists, shopping_list_items';
    RAISE NOTICE '   - ai_meal_suggestions, nutrition_reports, user_dietary_preferences';
    RAISE NOTICE '   - nutrition_badges, user_nutrition_badges, barcode_foods';
    RAISE NOTICE '   - RLS policies enabled';
    RAISE NOTICE '   - Auto-calculation triggers configured';
    RAISE NOTICE '   - 8 nutrition badges seeded';
END $$;
